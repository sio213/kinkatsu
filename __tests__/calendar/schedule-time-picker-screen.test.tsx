...
describe('差し替えモード（PR10-6b、replaceReminderId等がparamsに渡る場合）', () => {
  ...
  test.each([
    ...
    ['範囲外(負数)', '-1'],
    ['小数', '7.5'],
  ])('replaceHourが不正(%s)なら時のみ18にフォールバックする(分はreplaceMinuteの値を維持)', (_label, invalidHour) => {
    mockUseLocalSearchParams.mockReturnValue({
      dateKey: '2026-07-25',
      routineId: '10',
      routineName: '背中の日',
      replaceReminderId: '1',
      replaceHour: invalidHour,
      replaceMinute: '30',
    });
    const root = render();
    const picker = root.findByType(DateTimePicker);
  ...
  test.each([
    ...
    ['範囲外(負数)', '-1'],
    ['小数', '30.5'],
  ])('replaceMinuteが不正(%s)なら分のみ0にフォールバックする(時はreplaceHourの値を維持)', (_label, invalidMinute) => {
    mockUseLocalSearchParams.mockReturnValue({
      dateKey: '2026-07-25',
      routineId: '10',
      routineName: '背中の日',
  ...
  test('確定を押すと、skipReminderOccurrence(元のreminderId, 選択日)→createScheduledWorkoutの順で呼ばれる', async () => {
    ...
    // 「今回だけ差し替え」も通常のルーティン予定作成と同じく目標セット編集画面へpushする（PR7で統一）
    expect(mockPush).toHaveBeenCalledWith({
      pathname: '/calendar/schedule-workout-edit',
      params: { scheduledWorkoutId: '1' },
    });
  ...
  test('巻き戻し(unskipReminderOccurrence)自体が失敗しても、握りつぶしてエラーAlertは通常通り表示し、dismiss/pushもされない', async () => {
    mockCreateScheduledWorkout.mockRejectedValueOnce(new Error('fail'));
  ...
  test('notificationSuppressed: trueの通常ケースでは、差し替え完了時にAlertを出さずそのままdismissし、目標セット編集画面へpushする', async () => {
  ...
  test('過去時刻の場合、警告Alert経由でOKを押した後にdismiss(2)してから目標セット編集画面へpushする（@tester指摘: 差し替え×過去時刻×自動遷移の組み合わせが未検証だった）', async () => {
  ...
  test.each([
    ...
    'replaceReminderIdが不正(%s)な場合は差し替えモードとして扱わず、通常モードのボタンラベル・挙動になる(@reviewer指摘: routineId/dateKeyと同様の検証が無くNaNのままDB書き込みを試みうる問題)',
    (_label, replaceReminderId) => {
      mockUseLocalSearchParams.mockReturnValue({
        dateKey: '2026-07-25',
        routineId: '10',
        routineName: '背中の日',
      ...
      const root = render();
      expect(findByLabel(root, 'この時刻で追加')).toBeDefined();
      expect(findByLabel(root, 'この時刻で差し替え')).toBeUndefined();
    },
  );

  test('replaceReminderIdが不正な場合、確定してもskipReminderOccurrenceは呼ばれずcreateScheduledWorkoutだけが実行される', async () => {
    mockUseLocalSearchParams.mockReturnValue({
      dateKey: '2026-07-25',
      routineId: '10',
      routineName: '背中の日',
      replaceReminderId: 'abc',
      replaceHour: '7',
      replaceMinute: '30',
    });
    const root = render();
    ...
    // isReplaceModeがfalse扱いになるため、デフォルト時刻も通常モードの18:00になる
    expect(mockCreateScheduledWorkout).toHaveBeenCalledWith(10, '背中の日', '2026-07-25', 18, 0);
  });
});
...
describe('直接追加モード（exerciseIdsパラメータ、schedule-exercise-picker経由、2026-07-20）', () => {
  ...
  test('ヘッダーのサブタイトルは種目名から合成したタイトルになる（先頭の種目名 他N種目）', () => {
    const root = render();
    expect(root.findByProps({ children: '7月25日（土）・ベンチプレス 他1種目' })).toBeDefined();
  });
