...
describe('差し替えモード（PR10-6b、replaceReminderId等がparamsに渡る場合）', () => {
  beforeEach(() => {
    ...
    mockUseRoutines.mockReturnValue({ routines: [baseRoutine({ id: 20, name: '脚の日' })] });
    mockUseRoutineExerciseSummaries.mockReturnValue(new Map([[20, { exerciseCount: 3, categories: ['leg'] }]]));
  });

  test('見出しが「差し替えるルーティンを選択」になり、サブタイトルに元のルーティン名を含む', () => {
    const root = render();
    expect(root.findByProps({ children: '差し替えるルーティンを選択' })).toBeDefined();
    expect(root.findByProps({ children: '7月25日（土）・「胸の日」を差し替え' })).toBeDefined();
  });

  test('カードをタップすると、replaceReminderId/replaceHour/replaceMinuteを引き継いだまま時刻選択画面へ遷移する（replaceRoutineNameは引き継がない）', () => {
    const root = render();
    const card = findCardByLabel(root, '脚の日')!;
    act(() => {
      card.props.onPress();
...
test('replaceReminderIdが無い通常モードでは、見出しは従来通り「ルーティンを選択」のまま', () => {
  const root = render();
