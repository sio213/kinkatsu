...
describe('addExercisesのlastAddedAt(exercise-edit.tsxの自動スクロール用通知)', () => {
  test('追加された最初の種目のindex(=追加前のexercises.length)をセットする', () => {
    const { hydrate, addExercises } = useRoutineDraftStore.getState();
    hydrate([makeDraftExercise(1)]);
    addExercises([makeDraftExercise(2), makeDraftExercise(3)]);

    expect(useRoutineDraftStore.getState().lastAddedAt?.index).toBe(1);
  });

  test('空配列を渡した場合はlastAddedAtを変えない(追加が実際に無いのに通知しない)', () => {
    const { hydrate, addExercises } = useRoutineDraftStore.getState();
    hydrate([makeDraftExercise(1)]);
    expect(useRoutineDraftStore.getState().lastAddedAt).toBeNull();
  ...
  test('連続で呼ぶたびに異なるtokenになる(同じ画面インスタンス内で複数回追加されても検知できる)', () => {
    const { addExercises } = useRoutineDraftStore.getState();
    addExercises([makeDraftExercise(1)]);
    const first = useRoutineDraftStore.getState().lastAddedAt;

    addExercises([makeDraftExercise(2)]);
    const second = useRoutineDraftStore.getState().lastAddedAt;
  ...
  test('resetでnullに戻る', () => {
    const { addExercises, reset } = useRoutineDraftStore.getState();
    addExercises([makeDraftExercise(1)]);
    expect(useRoutineDraftStore.getState().lastAddedAt).not.toBeNull();
  ...
  test('hydrateでもnullに戻る(reset()を挟まず別ルーティンの編集を開始した場合に、前の編集セッションの' +
    '古い通知がexercise-edit.tsxの自動スクロールへ誤って残らないようにするため)', () => {
    const { addExercises, hydrate } = useRoutineDraftStore.getState();
    addExercises([makeDraftExercise(1)]);
    expect(useRoutineDraftStore.getState().lastAddedAt).not.toBeNull();

    hydrate([makeDraftExercise(10), makeDraftExercise(20)]);
