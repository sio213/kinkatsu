...
test('useExercisesWithHistoryのSetに含まれる種目だけ「過去の記録から読み込み」が有効になる', () => {
  ...
  const items = root
    .findAllByType(TouchableOpacity)
    .filter((t) => t.props.accessibilityLabel === '過去の記録から読み込み');
...
describe('ヘッダー⋮メニュー: 種目を追加', () => {
  test('画面下部のボタンと同じ種目追加ピッカーへ、returnToパラメータ付きで遷移する', () => {
    const root = render();
    const menuBtn = root
      .findAllByType(TouchableOpacity)
  ...
  test('他のメニュー項目より上に表示される', () => {
    ...
    const labels = root
      ...
      .filter((text) => ['種目を追加', '過去の記録から読み込み', '種目を並び替え'].includes(text));
    expect(labels[0]).toBe('種目を追加');
  });
});
...
describe('ヘッダー⋮メニュー: 過去の記録から読み込み', () => {
  test('選択すると/routine/session-history-pickerへ遷移する(件数によらず常に有効)', () => {
    ...
    const historyBtn = findButtonByLabel(root, '過去の記録から読み込み')!;
...
describe('種目追加後の自動スクロール・自動フォーカス(過去の記録から読み込み・種目を追加ピッカーから戻った直後、追加された最初のカードまでスクロールし最初のセットの入力欄にフォーカスする。トレーニング中画面のcardRefsRef/focusFirstSetと同じ考え方)', () => {
  ...
  test('過去の記録から読み込み等で複数の種目が一度に追加された場合は、キーボードを自動で開かない(スクロールのみ行う)', () => {
    act(() => {
      useRoutineDraftStore.getState().hydrate([makeExercise(1)]);
    });
    const root = render();
