...
test('進行中セッションが既にある場合は、カードをタップしても新規セッションを作らずそちらへ遷移する', async () => {
...
test('進行中セッションがある場合、一覧上部に再開バナーを表示し、タップするとそちらへ遷移する', async () => {
  const activeSession = { id: 9, startedAt: 0, endedAt: null };
  mockUseWorkoutSessions.mockReturnValue(baseSessions({ sessions: [activeSession], activeSession }));
  const root = render();

  const banner = findByAccessibilityLabel(root, '進行中のトレーニングを再開する')!;
  await act(async () => {
    await banner.props.onPress();
  });
...
test('進行中セッションが無い場合は再開バナーを表示しない', () => {
  const root = render();
  expect(findByAccessibilityLabel(root, '進行中のトレーニングを再開する')).toBeUndefined();
});
