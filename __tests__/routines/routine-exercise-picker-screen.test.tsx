...
test('1件選択して確定すると、buildInitialRoutineSetsが呼ばれドラフトストアに追加され種目編集画面へ遷移する', async () => {
  const root = render();
  act(() => {
  ...
  expect(mockReplace).toHaveBeenCalledWith('/routine/exercise-edit');
});
...
test('returnTo=exercise-editのとき(テンプレートセット編集画面自身の「種目を追加」から開いた場合)、確定後はreplaceせずback()する(編集画面がスタックに二重に積まれるのを防ぐ)', async () => {
  mockUseLocalSearchParams.mockReturnValue({ returnTo: 'exercise-edit' });
  const root = render();
  act(() => {
    root.findByProps({ accessibilityLabel: benchPressLabel }).props.onPress();
  });
