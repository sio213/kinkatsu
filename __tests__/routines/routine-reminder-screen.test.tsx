...
// デザイン案どおりキャンセルボタンは持たない(戻る操作はヘッダーの標準の戻るボタンに任せる)。
// 「設定を押すと...」テストが唯一のフッターボタン(保存)であることを兼ねて確認している
test('キャンセルボタンは表示されない(デザイン案どおり戻るボタンのみ)', () => {
  const root = render();
  expect(() => findButtonByLabel(root, 'キャンセル')).not.toThrow();
