...
test('種目行の削除ボタンを押すと、ドラフトストアとフォーム両方から取り除かれる', () => {
  const root = render();
  addExercisesAfterRender([makeDraftExercise(5), makeDraftExercise(6)]);

  const removeBtn = root.findByProps({ accessibilityLabel: '種目5を削除' });
  act(() => {
...
test('種目行をタップすると準備中Alertが表示される（テンプレートセット編集画面は未実装）', () => {
  const root = render();
  addExercisesAfterRender([makeDraftExercise(5)]);
  ...
  expect(Alert.alert).toHaveBeenCalledWith('準備中', 'この機能は近日公開予定です。');
});
