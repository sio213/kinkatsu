...
// リマインダートグルはデフォルトON+未設定のため、そのまま保存しようとするとバリデーション
// エラーになる(routineFormSchemaのrefine)。名前・種目のバリデーションだけを見たいテストでは
// 事前にOFFにしておく(render()後でないとマウント時resetで打ち消されるため、他の
// after-render系ヘルパーと同じ扱いにする)
function disableReminderAfterRender() {
  act(() => {
...
test('種目行をタップするとテンプレートセット編集画面へ、タップした種目のindexがfocusIndexとして渡される', () => {
  const root = render();
  addExercisesAfterRender([makeDraftExercise(1), makeDraftExercise(5)]);
...
describe('リマインダーセクション', () => {
  test('リマインダーがデフォルトON+未設定のまま保存すると、エラーが表示されcreateRoutineは呼ばれない', async () => {
    const root = render();
    addExercisesAfterRender([makeDraftExercise(5)]);
    const nameInput = root.findAllByType(TextInput)[0];
    act(() => {
  ...
  test('リマインダーの設定行を押すと/routine/reminderへ遷移する', () => {
    ...
    expect(mockPush).toHaveBeenCalledWith('/routine/reminder');
  });

  test('リマインダーをON+設定済みにして保存すると、createRoutineの第2引数にreminderPlanが渡る', async () => {
    const root = render();
    addExercisesAfterRender([makeDraftExercise(5)]);
    act(() => {
    ...
    expect(mockCreateRoutine).toHaveBeenCalledWith(
      ...
      { enabled: true, input: expect.objectContaining({ kind: 'interval', hour: 18, minute: 0 }) },
    );
    expect(mockBack).toHaveBeenCalled();
  });
});
