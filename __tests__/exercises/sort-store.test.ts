...
test('pickerSortByの初期値はfrequent（種目タブのlistSortByとは独立）', () => {
...
test('setPickerSortByした値はAsyncStorageに永続化される', async () => {
  ...
  const raw = await AsyncStorage.getItem(STORAGE_KEY);
  expect(JSON.parse(raw!).state.pickerSortBy).toBe('recent');
});

test('merge: 永続化データにpickerSortByの有効な値があればそれを採用する', async () => {
  await AsyncStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({ state: { pickerSortBy: 'recent' }, version: 0 }),
  );
  await useExerciseSortStore.persist.rehydrate();
  expect(useExerciseSortStore.getState().pickerSortBy).toBe('recent');
});

test('merge: persistedが存在しない（初回起動）場合はpickerSortByもデフォルト(frequent)のまま', async () => {
  await expect(useExerciseSortStore.persist.rehydrate()).resolves.not.toThrow();
  expect(useExerciseSortStore.getState().pickerSortBy).toBe('frequent');
});
...
test.each([
  ...
  ['オブジェクト', {}],
  ['空文字', ''],
])('merge: pickerSortByの不正な値(%s)はfrequentにフォールバックする', async (_label, badValue) => {
  await AsyncStorage.setItem(
    STORAGE_KEY,
...
test('swapSortByの初期値はfrequent（種目タブ・pickerとは独立）', () => {
  expect(useExerciseSortStore.getState().swapSortBy).toBe('frequent');
});

test('setSwapSortByは他の軸に影響しない', () => {
  useExerciseSortStore.getState().setSwapSortBy('recent');
  expect(useExerciseSortStore.getState().swapSortBy).toBe('recent');
  expect(useExerciseSortStore.getState().listSortBy).toBe('category');
  expect(useExerciseSortStore.getState().pickerSortBy).toBe('frequent');
});

test('setSwapSortByした値はAsyncStorageに永続化される', async () => {
  useExerciseSortStore.getState().setSwapSortBy('recent');
  await new Promise((resolve) => setTimeout(resolve, 0));

  const raw = await AsyncStorage.getItem(STORAGE_KEY);
  expect(JSON.parse(raw!).state.swapSortBy).toBe('recent');
});

test('merge: 永続化データにswapSortByの有効な値があればそれを採用する', async () => {
  await AsyncStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({ state: { swapSortBy: 'recent' }, version: 0 }),
  );
  await useExerciseSortStore.persist.rehydrate();
  expect(useExerciseSortStore.getState().swapSortBy).toBe('recent');
});

test('merge: persistedが存在しない（初回起動）場合はswapSortByもデフォルト(frequent)のまま', async () => {
  await expect(useExerciseSortStore.persist.rehydrate()).resolves.not.toThrow();
  expect(useExerciseSortStore.getState().swapSortBy).toBe('frequent');
});
...
test.each([
  ...
  ['オブジェクト', {}],
  ['空文字', ''],
])('merge: swapSortByの不正な値(%s)はfrequentにフォールバックする', async (_label, badValue) => {
  await AsyncStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({ state: { swapSortBy: badValue }, version: 0 }),
  );
  await useExerciseSortStore.persist.rehydrate();
  expect(useExerciseSortStore.getState().swapSortBy).toBe('frequent');
});
