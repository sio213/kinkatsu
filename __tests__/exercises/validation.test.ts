...
describe('exerciseSchema', () => {
  test('正常系: name/category/noteすべて有効な値', () => {
    const r = exerciseSchema.safeParse({ name: 'ベンチプレス', category: '胸', note: 'メモ' });
    expect(r.success).toBe(true);
    if (r.success) {
      expect(r.data).toEqual({ name: 'ベンチプレス', category: '胸', note: 'メモ' });
    }
  });

  describe('name', () => {
    test('空文字は「種目名を入力してください」', () => {
      const r = exerciseSchema.safeParse({ name: '', category: '胸', note: '' });
      expect(r.success).toBe(false);
      if (!r.success) expect(r.error.issues[0].message).toBe('種目名を入力してください');
    });

    test('前後の空白はtrimされて保存される', () => {
      const r = exerciseSchema.safeParse({ name: '  ベンチ  ', category: '胸', note: '' });
      expect(r.success).toBe(true);
  ...
  describe('note', () => {
    test('空文字はnullに変換される', () => {
      const r = exerciseSchema.safeParse({ name: 'a', category: '胸', note: '' });
    ...
    test('前後の空白はtrimされる', () => {
      const r = exerciseSchema.safeParse({ name: 'a', category: '胸', note: '  フォーム注意  ' });
      expect(r.success && r.data.note).toBe('フォーム注意');
    });

    test('nullを直接渡してもそのまま通る', () => {
      const r = exerciseSchema.safeParse({ name: 'a', category: '胸', note: null });
      expect(r.success && r.data.note).toBeNull();
    });
  });
});
