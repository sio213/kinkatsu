...
test('バリデーションエラー時、scrollToFirstErrorがエラーのフィールド名付きで呼ばれる(自動スクロール機能の配線確認、内蔵の保存ボタン経由)', async () => {
  ...
  expect(mockScrollToFirstError.mock.calls[0][0]).toEqual(expect.objectContaining({ weekdays: expect.anything() }));
});

test('有効な値で保存が成功した場合はscrollToFirstErrorは呼ばれない', async () => {
  mockScrollToFirstError.mockClear();
  const { root, onSubmit } = await render();
...
test('週次で曜日を選ばずエラーを出した後、繰り返し種別を毎日に切り替えると保存ボタンが再び押せる', async () => {
  // 実際のバグ報告の再現: kindを切り替えるsetValueにshouldValidateが無いと、
  // 前のkind(weekly)向けのエラー(weekdays)がerrorsオブジェクトに残り続け、
  // hasErrors(=Object.keys(errors).length>0)がtrueのままになる。submitDisabledは
  // isSubmitted&&hasErrorsで決まるため、以後どのkindに変えても保存ボタンが
  // disabled={true}のまま固まってしまう(呼び出し側のonPressを直接呼ぶテストヘルパーでは
  // 検知できないため、ここではdisabledプロパティ自体を検証する)
  const { root, onSubmit } = await render();

  await press(root, '毎週');
  await submit(root);
  expect(allTexts(root)).toContain('曜日を1つ以上選択してください');
  expect(findChipByLabel(root, '保存')!.props.disabled).toBe(true);

  await press(root, '毎日');

  expect(findChipByLabel(root, '保存')!.props.disabled).toBe(false);
  expect(allTexts(root)).not.toContain('曜日を1つ以上選択してください');

  await submit(root);
  expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ kind: 'interval' }));
});

test('毎月(第N曜日)で曜日を選ばずエラーを出した後、日付指定モードに切り替えると保存ボタンが再び押せる', async () => {
  const { root, onSubmit } = await render();
  ...
  await press(root, '日付');
  expect(findChipByLabel(root, '保存')!.props.disabled).toBe(true); // 日付側もまだ未選択のため引き続きエラー

  await press(root, 1);
  expect(findChipByLabel(root, '保存')!.props.disabled).toBe(false);

  await submit(root);
  expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ kind: 'monthly', monthdays: [1] }));
});
