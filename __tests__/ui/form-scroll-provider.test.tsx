...
test('複数のエラー項目のうち、実測Y座標が一番小さい(画面上で一番上の)項目までスクロールする', async () => {
  ...
  const scrollRef = { current: { getInnerViewRef: () => ({}), scrollTo } } as any;
...
test('registerはされているがref.currentがnull(レイアウト未確定・対象消滅等)の項目は無視される', async () => {
  const scrollTo = jest.fn();
  const scrollRef = { current: { getInnerViewRef: () => ({}), scrollTo } } as any;
  let trigger: ((errors: Record<string, unknown>) => void) | null = null;

  function NullRefField({ name }: { name: string }) {
    const ref = useRef<View>(null); // currentは常にnull
    useFormScrollRegistration(name, ref);
    return null;
  }
  ...
  act(() => {
    create(
      <FormScrollProvider scrollRef={scrollRef}>
        <NullRefField name="ghost" />
        <TestField name="name" y={80} />
        <TestConsumer onReady={(fn) => (trigger = fn)} />
      </FormScrollProvider>,
    );
  });
  ...
  expect(scrollTo).toHaveBeenCalledWith({ y: 80 - 16, animated: true });
});
