...
test('取得成功かつ1件以上あれば注意バナーを表示する', async () => {
  const root = await renderResolved([entry1]);
  expect(root.findByProps({ children: '記録から読み込む' })).toBeDefined();
...
test('自己ベストのカードにだけ「自己ベスト」バッジが付く', async () => {
  ...
  expect(
    root.findByProps({ accessible: true, accessibilityLabel: '7月1日（水）、60kg×10・60kg×8' }),
  ).toBeDefined();
  expect(
    root.findByProps({ accessible: true, accessibilityLabel: '6月1日（月）、自己ベスト、80kg×5' }),
  ).toBeDefined();
});
