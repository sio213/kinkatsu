...
test('完了済みのセットを✓で編集に戻すと、DB値そのままでonDraftChangeが呼ばれる（addSet側のコピー元として機能させるため）', async () => {
  const onDraftChange = jest.fn();
  const onReopen = jest.fn().mockResolvedValue(undefined);
  const set = { id: 5, setNumber: 1, weight: 62.5, reps: 8, completedAt: 123 } as any;
  const root = render({
    set,
    measurementType: 'weight_reps',
    onSave: jest.fn(),
  ...
  expect(onDraftChange).toHaveBeenCalledWith(5, { weight: '62.5', reps: '8' });
});
...
test('time計測タイプの完了済みセットは分割されず単一の読み取り専用表示になる', () => {
  ...
  expect(inputs[0].props.editable).toBe(false);
  expect(inputs[0].props.value).toBe('1:30');
});

test('time計測タイプの完了済みセットを✓で編集に戻すと、分・秒欄にDB値が再分割表示される', async () => {
  const onReopen = jest.fn().mockResolvedValue(undefined);
  const set = { id: 1, setNumber: 1, durationSeconds: 90, completedAt: 123 } as any;
  let instance!: ReturnType<typeof create>;
  act(() => {
    instance = create(
      <SetRow exerciseName="ベンチプレス" set={set} measurementType="time" onSave={jest.fn()} onReopen={onReopen} />,
    );
  });
  const root = instance.root;
  ...
  // 実アプリではonReopen後にDB(completedAt: null)へ更新されたsetが親から再度渡される。
  // テストではその再レンダーを明示的にシミュレートする
  const reopenedSet = { ...set, completedAt: null };
  act(() => {
    instance.update(
      <SetRow exerciseName="ベンチプレス" set={reopenedSet} measurementType="time" onSave={jest.fn()} onReopen={onReopen} />,
    );
  });
