...
test('セッションカードをタップすると読み込む種目を選ぶ画面へ遷移する', async () => {
  ...
  expect(mockPush).toHaveBeenCalledWith({
    ...
    params: { sessionId: '99', sourceSessionId: '1' },
  });
});
...
test('複数セッションが表示された状態で、押したカードに対応するsourceSessionIdが渡る（先頭固定になっていないことの確認）', async () => {
  ...
  expect(mockPush).toHaveBeenCalledWith({
    pathname: '/workout/session-history-load',
    params: { sessionId: '99', sourceSessionId: '2' },
  });
});
...
test('同じ暦日に2セッションある場合、両方のカードが表示され開始時刻で区別できる', async () => {
  ...
  expect(root.findByProps({ children: '19:00' })).toBeDefined();
  expect(root.findByProps({ children: '07:00' })).toBeDefined();
});

test('同じ暦日でも他の日と重複していないセッションは開始時刻を表示しない', async () => {
  const root = await renderResolved([chestSession, legSession]);
