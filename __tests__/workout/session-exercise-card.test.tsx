...
jest.mock('@/lib/workout/session', () => ({
  ...
  undoPrefill: (...args: unknown[]) => mockUndoPrefill(...args),
}));
...
describe('前回のセットのプリフィル通知（カード内スナックバー、デザイン案P14）', () => {
  test('justPrefilled=falseの間はスナックバーを表示しない', () => {
    const root = render({ exercise, sessionId: 1, sets: [], justPrefilled: false });
    expect(() => root.findByProps({ accessibilityRole: 'alert' })).toThrow();
  });

  test('justPrefilled=trueのとき「前回のセットを挿入」のスナックバーを表示する', () => {
    const root = render({ exercise, sessionId: 1, sets: [], justPrefilled: true });
    expect(root.findByProps({ children: '前回のセットを挿入' })).toBeTruthy();
  });

  test('「元に戻す」を押すとundoPrefillが呼ばれ、onDismissPrefillで自身のsessionExerciseIdを通知する', async () => {
    mockUndoPrefill.mockResolvedValue(undefined);
    const root = render({ exercise, sessionId: 1, sets: [], justPrefilled: true });

    const undoButton = root.findByProps({ accessibilityLabel: '元に戻す' });
    await act(async () => {
      undoButton.props.onPress();
    });

    expect(mockUndoPrefill).toHaveBeenCalledWith({
      sessionId: 1,
  ...
  test('undoPrefillが失敗した場合はエラーAlertを表示する（スナックバー自体は楽観的に閉じる）', async () => {
    mockUndoPrefill.mockRejectedValue(new Error('fail'));
    const root = render({ exercise, sessionId: 1, sets: [], justPrefilled: true });
    ...
    expect(mockOnDismissPrefill).toHaveBeenCalledWith(exercise.sessionExerciseId);
    expect(Alert.alert).toHaveBeenCalledWith('エラー', '元に戻せませんでした。');
  });

  describe('isVisible（画面外では自動消滅タイマーを開始しない）', () => {
    beforeEach(() => {
      jest.useFakeTimers();
    });
    afterEach(() => {
    ...
    test('isVisible=falseの間はどれだけ待ってもonDismissPrefillが呼ばれない（見えないまま自動消滅しない）', () => {
      render({ exercise, sessionId: 1, sets: [], justPrefilled: true, isVisible: false });
    ...
    test('isVisibleがfalse→trueになった時点から数秒後にonDismissPrefillが呼ばれる', () => {
      let instance!: ReturnType<typeof create>;
      act(() => {
        instance = create(
          <SessionExerciseCard
