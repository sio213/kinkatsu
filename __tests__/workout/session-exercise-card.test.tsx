...
test('「種目を入れ替え」をタップした時、いずれかのセットに記録済みの値があればhasRecordedData: trueを渡す', () => {
  const sets = [{ id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: null }] as any;
  const root = render({ exercise, sessionId: 1, sets });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
...
test('「種目を入れ替え」で渡すcurrentMeasurementTypeは種目ごとの実際の値になる（ハードコード検知用）', () => {
  const timeExercise = { ...exercise, id: 20, sessionExerciseId: 600, measurementType: 'time' } as any;
  const root = render({ exercise: timeExercise, sessionId: 1, sets: [] });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
  });
