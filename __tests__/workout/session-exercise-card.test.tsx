...
describe('前回の値のクリア導線（ゴースト値方式、デザイン案P1）', () => {
  ...
  test('ゴースト行が1件も無くなると、onDismissPrefillで自身のsessionExerciseIdを親に通知する', () => {
  ...
  test('✓確定済みのセットだけならゴースト行とみなさず、onDismissPrefillを呼ぶ', () => {
    const confirmedSets = [{ id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: 123 }] as any;
    render({ exercise, sessionId: 1, sets: confirmedSets, justPrefilled: true });
    expect(mockOnDismissPrefill).toHaveBeenCalledWith(exercise.sessionExerciseId);
  });
});
