...
test('collapsed=trueのときは折りたたみ表示になり、セット数のサマリーが出てbodyは非表示スタイルになる', () => {
  ...
  const header = findHeaderToggle(root);
  expect(header.props.accessibilityLabel).toBe('ベンチプレス、2セット・60kg×10、展開する');
  expect(header.props.accessibilityState).toEqual({ expanded: false });
  const summaryText = root
    .findAllByType(Text)
    .find((t) => [t.props.children].flat().join('') === '2セット・60kg×10');
...
test('⋮メニューを開くと種目を入れ替え・上へ移動・下へ移動・過去の記録から読み込み・削除の5項目が表示される', () => {
  const root = render({ exercise, sessionId: 1, sets: [] });
  act(() => {
...
test('⋮メニューを開くとKeyboard.dismiss()が呼ばれる（他画面から戻った時にフォーカス中の入力欄へ自動スクロールする不具合の回帰防止）', () => {
...
test('hasHistory=falseのとき「過去の記録から読み込み」が無効になり、タップしても遷移しない', () => {
  const root = render({ exercise, sessionId: 1, sets: [], hasHistory: false });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
  });
  const item = findMenuItem(root, '過去の記録から読み込み')!;
...
test('hasHistory=trueのとき「過去の記録から読み込み」は有効', () => {
  const root = render({ exercise, sessionId: 1, sets: [], hasHistory: true });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
  });
  expect(findMenuItem(root, '過去の記録から読み込み')!.props.disabled).toBe(false);
});
...
test('「過去の記録から読み込み」をタップすると、記録から読み込む画面へこのカード・種目の情報を渡して遷移する', () => {
  const root = render({ exercise, sessionId: 1, sets: [] });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
  });
...
test('「過去の記録から読み込み」をタップした時、いずれかのセットが✓確定済みであればhasRecordedData: trueを渡す', () => {
  const sets = [{ id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: Date.now() }] as any;
  const root = render({ exercise, sessionId: 1, sets });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
...
describe('prefilledSetIdsの行への伝播とfocusFirstSet()（ゴースト値方式の簡素化後、デザイン案P1）', () => {
  test('prefilledSetIdsに含まれるセットの行はゴースト表示になる（✓のaccessibilityLabelで判定）', () => {
    const sets = [{ id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: null }] as any;
    const root = render({ exercise, sessionId: 1, sets, prefilledSetIds: [1] });
    const checkbox = root
      .findAllByType(TouchableOpacity)
      .find((t) => t.props.accessibilityRole === 'checkbox')!;
    expect(checkbox.props.accessibilityLabel).toBe('ベンチプレス セット1、未確認の値');
  });

  test('prefilledSetIdsに含まれないセットの行は値があってもゴースト表示にならない（セット追加で複製された行など）', () => {
    const sets = [{ id: 2, setNumber: 1, weight: 60, reps: 10, completedAt: null }] as any;
    const root = render({ exercise, sessionId: 1, sets, prefilledSetIds: [1] });
    const checkbox = root
      .findAllByType(TouchableOpacity)
      .find((t) => t.props.accessibilityRole === 'checkbox')!;
    expect(checkbox.props.accessibilityLabel).toBe('ベンチプレス セット1');
  });

  test('ref経由のfocusFirstSet()で先頭セットの最初の入力欄にだけフォーカスする（宣言的autoFocusは画面遷移アニメーション中にキーボードが被さる問題があるため使わない）', () => {
    const sets = [
      { id: 1, setNumber: 1, weight: null, reps: null, completedAt: null },
      { id: 2, setNumber: 2, weight: null, reps: null, completedAt: null },
    ] as any;
    const ref = React.createRef<SessionExerciseCardHandle>();
    let instance!: ReturnType<typeof create>;
    act(() => {
      instance = create(
        <SessionExerciseCard
  ...
  test('カード内の1セットでも✓確定されると、同じカードの残りのゴースト行も含めて全て通常色に戻る', () => {
    const initialSets = [
      { id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: null },
      { id: 2, setNumber: 2, weight: 60, reps: 8, completedAt: null },
    ] as any;
    let instance!: ReturnType<typeof create>;
    act(() => {
      instance = create(
    ...
    const checkboxesBefore = instance.root
      .findAllByType(TouchableOpacity)
      .filter((t) => t.props.accessibilityRole === 'checkbox');
    expect(checkboxesBefore.every((c) => c.props.accessibilityLabel.includes('未確認'))).toBe(true);

    // set1だけ✓確定された状態を再現（live queryの更新）
    const confirmedSets = [{ ...initialSets[0], completedAt: 123 }, initialSets[1]];
    act(() => {
      instance.update(
        <SessionExerciseCard
          exercise={exercise}
    ...
    const checkboxesAfter = instance.root
      .findAllByType(TouchableOpacity)
      .filter((t) => t.props.accessibilityRole === 'checkbox');
    expect(checkboxesAfter.every((c) => !c.props.accessibilityLabel.includes('未確認'))).toBe(true);
  });

  test('✓確定後にreopenして未確定に戻っても、カード内の他の行を含めゴースト表示は復活しない', () => {
    const initialSets = [
      { id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: 123 },
      { id: 2, setNumber: 2, weight: 60, reps: 8, completedAt: null },
    ] as any;
    let instance!: ReturnType<typeof create>;
    act(() => {
    ...
    // set1をreopenして未確定に戻した状態を再現（live queryの更新）
    const reopenedSets = [{ ...initialSets[0], completedAt: null }, initialSets[1]];
    act(() => {
      instance.update(
        <SessionExerciseCard
          exercise={exercise}
  ...
  test('カード内の1セットの値を変更しただけ（✓は未タップ）でも、同じカードの残りのゴースト行も含めて全て通常色に戻る', () => {
    const sets = [
      { id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: null },
      { id: 2, setNumber: 2, weight: 60, reps: 8, completedAt: null },
    ] as any;
    const root = render({ exercise, sessionId: 1, sets, prefilledSetIds: [1, 2] });

    const checkboxesBefore = root
      .findAllByType(TouchableOpacity)
      .filter((t) => t.props.accessibilityRole === 'checkbox');
    expect(checkboxesBefore.every((c) => c.props.accessibilityLabel.includes('未確認'))).toBe(true);

    // 1セット目の重量欄だけ編集する（✓はまだ押さない）
    const inputs = root.findAllByType(TextInput);
    act(() => {
      inputs[0].props.onChangeText('62.5');
    });

    const checkboxesAfter = root
      .findAllByType(TouchableOpacity)
      .filter((t) => t.props.accessibilityRole === 'checkbox');
    expect(checkboxesAfter.every((c) => !c.props.accessibilityLabel.includes('未確認'))).toBe(true);
  });

  test('✓確定済みのカードで種目を入れ替え/過去記録を読み込むと、新しく入った未確認セットはゴースト表示に戻る（同じカードはアンマウントされずcardReviewedのラッチが残っていたバグの回帰防止）', () => {
    const confirmedSets = [{ id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: 123 }] as any;
    let instance!: ReturnType<typeof create>;
    act(() => {
      instance = create(
        <SessionExerciseCard
    ...
    // 種目入れ替え/履歴読み込みは同じsessionExerciseId（＝同じカード）のまま、
    // 中身のセットだけ新しい未確認プリフィル値に差し替わる（liveQueryの更新）
    const swappedSets = [{ id: 10, setNumber: 1, weight: 80, reps: 5, completedAt: null }] as any;
    act(() => {
      instance.update(
        <SessionExerciseCard
          exercise={exercise}
