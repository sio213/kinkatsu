...
test('⋮メニューを開くと種目を入れ替え・上へ移動・下へ移動・過去の記録から読み込む・削除の5項目が表示される', () => {
  const root = render({ exercise, sessionId: 1, sets: [] });
  act(() => {
  ...
  expect(findMenuItem(root, '過去の記録から読み込む')).toBeDefined();
...
test('hasHistory=falseのとき「過去の記録から読み込む」が無効になり、タップしても遷移しない', () => {
...
test('hasHistory=trueのとき「過去の記録から読み込む」は有効', () => {
  const root = render({ exercise, sessionId: 1, sets: [], hasHistory: true });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
  });
  expect(findMenuItem(root, '過去の記録から読み込む')!.props.disabled).toBe(false);
});
...
test('「過去の記録から読み込む」をタップすると、記録から読み込む画面へこのカード・種目の情報を渡して遷移する', () => {
  const root = render({ exercise, sessionId: 1, sets: [] });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
  });
...
test('「過去の記録から読み込む」をタップした時、いずれかのセットが✓確定済みであればhasRecordedData: trueを渡す', () => {
  const sets = [{ id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: Date.now() }] as any;
  const root = render({ exercise, sessionId: 1, sets });
  act(() => {
    findMenuTrigger(root)!.props.onPress();
