...
const mockOnDismissPrefill = jest.fn();
...
jest.mock('@/lib/workout/session', () => ({
  ...
  clearPrefill: (...args: unknown[]) => mockClearPrefill(...args),
}));
...
function render(
  ...
  const merged = {
    ...
    justPrefilled: false,
    onDismissPrefill: mockOnDismissPrefill,
    ...props,
  };
...
describe('前回の値のクリア導線（ゴースト値方式、デザイン案P1）', () => {
  const ghostSets = [{ id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: null }] as any;

  test('justPrefilled=falseの間はクリア導線を表示しない', () => {
    const root = render({ exercise, sessionId: 1, sets: ghostSets, justPrefilled: false });
    expect(() => root.findByProps({ accessibilityLabel: '前回の値をクリア' })).toThrow();
  });

  test('justPrefilled=trueでも、値の入った未確認行(ゴースト行)が無ければクリア導線を表示しない', () => {
    const root = render({ exercise, sessionId: 1, sets: [], justPrefilled: true });
    expect(() => root.findByProps({ accessibilityLabel: '前回の値をクリア' })).toThrow();
  });

  test('justPrefilled=trueかつゴースト行がある間は「前回の値をクリア」導線を表示する', () => {
    const root = render({ exercise, sessionId: 1, sets: ghostSets, justPrefilled: true });
    expect(root.findByProps({ accessibilityLabel: '前回の値をクリア' })).toBeTruthy();
  });

  test('「前回の値をクリア」を押すとclearPrefillが呼ばれる', async () => {
    mockClearPrefill.mockResolvedValue(undefined);
    const root = render({ exercise, sessionId: 1, sets: ghostSets, justPrefilled: true });

    const clearButton = root.findByProps({ accessibilityLabel: '前回の値をクリア' });
    await act(async () => {
      await clearButton.props.onPress();
    });

    expect(mockClearPrefill).toHaveBeenCalledWith({
      sessionId: 1,
  ...
  test('clearPrefillが失敗した場合はエラーAlertを表示する', async () => {
    mockClearPrefill.mockRejectedValue(new Error('fail'));
    const root = render({ exercise, sessionId: 1, sets: ghostSets, justPrefilled: true });
    ...
    expect(Alert.alert).toHaveBeenCalledWith('エラー', '前回の値をクリアできませんでした。');
  });

  // justPrefilled=trueになった直後の1レンダーだけsetsがまだ更新前（swap前）を指している
  // ことがあるため、「一度でも実際にゴースト行を観測してから空になった」場合だけ
  // onDismissPrefillを呼ぶ（session-exercise-card.tsxのhasSeenGhostRowsRef参照）。
  // 一度もゴースト行が無いまま（例えば最初のレンダーからsets=[]）では呼ばれない
  test('justPrefilledのままsetsが最初からゴースト行を含まない場合は、onDismissPrefillを呼ばない（stale live query対策）', () => {
    render({ exercise, sessionId: 1, sets: [], justPrefilled: true });
    expect(mockOnDismissPrefill).not.toHaveBeenCalled();
  });

  test('ゴースト行を一度観測した後、全て確認/編集されて無くなるとonDismissPrefillで自身のsessionExerciseIdを親に通知する', () => {
    const ghostSet = { id: 1, setNumber: 1, weight: 60, reps: 10, completedAt: null } as any;
    let instance!: ReturnType<typeof create>;
    act(() => {
      instance = create(
        <SessionExerciseCard
    ...
    const confirmedSet = { ...ghostSet, completedAt: 123 };
    act(() => {
      instance.update(
        <SessionExerciseCard
          exercise={exercise}
    ...
    expect(mockOnDismissPrefill).toHaveBeenCalledWith(exercise.sessionExerciseId);
  });
});
