...
// 表示が変わりflakyになる。entryの2日後に固定する（workout-screen.test.tsxと同じ対応）
...
test('日付・要約を表示し、自己ベストでなければバッジは表示しない', () => {
  ...
  expect(() => root.findByProps({ children: 'ベスト' })).toThrow();
});

test('6ヶ月前の記録は相対日付として「6ヶ月前」を表示する', () => {
  const oldEntry = { ...entry, startedAt: new Date(2026, 0, 4, 10, 0).getTime() };
  const { root } = render({ entry: oldEntry });
  expect(root.findByProps({ children: '6ヶ月前' })).toBeDefined();
});

test('2年以上前の記録は相対日付として「N年前」を表示する', () => {
  const veryOldEntry = { ...entry, startedAt: new Date(2024, 6, 1, 10, 0).getTime() };
  const { root } = render({ entry: veryOldEntry });
  expect(root.findByProps({ children: '2年前' })).toBeDefined();
});

test('isBestがtrueなら自己ベストバッジを表示する（表示文言は「ベスト」に短縮）', () => {
  const { root } = render({ isBest: true });
  expect(root.findByProps({ children: 'ベスト' })).toBeDefined();
});
...
test('日付・相対日付・自己ベスト・要約を1つの読み上げ単位（accessibilityLabel）にまとめる', () => {
  ...
  const infoView = root.findByProps({
    accessible: true,
    accessibilityLabel: '7月1日（水）、2日前、自己ベスト、60kg×10・60kg×8',
  });
