...
jest.mock('@/lib/workout/session', () => ({
  ...
  undoLoadHistory: (...args: unknown[]) => mockUndoLoadHistory(...args),
}));
...
test('別画面からnotifyHistoryLoadedが届くと下部にトースト（取り消す付き）が表示される', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  const root = render();

  act(() => {
    notifyHistoryLoaded({ sessionId: 1, sessionExerciseId: 100, dateLabel: '7/3', previousSnapshot: [] });
  });

  expect(root.findByProps({ children: '7/3の記録を読み込みました' })).toBeDefined();
  expect(findButtonByLabel(root, '取り消す')).toBeDefined();
});

test('別セッション宛のnotifyHistoryLoadedイベントは無視される', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  const root = render();

  act(() => {
    notifyHistoryLoaded({ sessionId: 999, sessionExerciseId: 100, dateLabel: '7/3', previousSnapshot: [] });
  });

  expect(() => root.findByProps({ children: '7/3の記録を読み込みました' })).toThrow();
});

test('「取り消す」を押すとundoLoadHistoryへ渡した通りのスナップショットで呼ばれ、トーストが消える', async () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  ...
  const undoBtn = findButtonByLabel(root, '取り消す')!;
  await act(async () => {
    undoBtn.props.onPress();
  });

  expect(mockUndoLoadHistory).toHaveBeenCalledWith(100, previousSnapshot);
  expect(() => root.findByProps({ children: '7/3の記録を読み込みました' })).toThrow();
});

test('undoLoadHistoryが失敗した場合はエラーAlertを表示する', async () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  mockUndoLoadHistory.mockRejectedValueOnce(new Error('fail'));
  const root = render();
  ...
  const undoBtn = findButtonByLabel(root, '取り消す')!;
  await act(async () => {
    await undoBtn.props.onPress();
  });

  expect(Alert.alert).toHaveBeenCalledWith('エラー', '取り消せませんでした。');
});

test('5秒経つとトーストが自動的に消える', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  const root = render();
