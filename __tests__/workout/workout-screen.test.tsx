...
test('⋮メニューに「過去の記録から読み込む」項目が表示される（種目カードのメニューと同じ文言）', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  const root = render();
...
test('「過去の記録から読み込む」をタップするとメニューを閉じ、過去のトレーニングを選ぶ画面へ遷移する', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  const root = render();
...
test('過去の記録編集モード（endedAt有り）でも「過去の記録から読み込む」項目が表示され、正しく遷移する', () => {
  const finishedSession = { id: 1, startedAt: FIXED_NOW - 5000, endedAt: FIXED_NOW };
  mockUseWorkoutSession.mockReturnValue({ session: finishedSession, loaded: true });
  const root = render();
...
test('⋮メニューに「ルーティンから読み込む」項目が「過去の記録から読み込む」より上に表示される', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  const root = render();
  ...
  expect(labels.indexOf('ルーティンから読み込む')).toBeGreaterThanOrEqual(0);
  expect(labels.indexOf('ルーティンから読み込む')).toBeLessThan(labels.indexOf('過去の記録から読み込む'));
});

test('「ルーティンから読み込む」をタップするとメニューを閉じ、ルーティンを選ぶ画面へ遷移する', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  const root = render();
  ...
  act(() => {
    findMenuItem(root, 'ルーティンから読み込む')!.props.onPress();
  });
...
test('保存済みのルーティンが無いときは「ルーティンから読み込む」が無効化され、押しても遷移しない', () => {
  mockUseWorkoutSession.mockReturnValue({ session: activeSession, loaded: true });
  ...
  const routineItem = findMenuItem(root, 'ルーティンから読み込む')!;
