...
describe('summarizeExerciseSets', () => {
  test('0件は「0セット」になる', () => {
    expect(summarizeExerciseSets('weight_reps', [])).toBe('0セット');
  });

  test('代表セットが決まる場合は「Nセット・{代表セット}」になる', () => {
    const sets = [set(60, 8), set(60, 8), set(55, 8)];
    expect(summarizeExerciseSets('weight_reps', sets)).toBe('3セット・60kg×8');
  });

  test('全セット未入力の場合は件数のみにフォールバックする', () => {
    const sets = [set(null, null)];
    expect(summarizeExerciseSets('weight_reps', sets)).toBe('1セット');
  });
