...
test('並び替えドロップダウンが表示され、デフォルトは「最近使った順」', () => {
  const root = render();
  const sortTrigger = root
    .findAllByType(TouchableOpacity)
    .find((t) => (t.props.accessibilityLabel as string)?.startsWith('並び替え: '));
  expect(sortTrigger!.props.accessibilityLabel).toBe('並び替え: 最近使った順');
});

test('使用実績が無い種目しか無い場合、デフォルトの「最近使った順」でも名前順にフォールバックする', () => {
  // benchPress/squatともに一度も記録が無い（usageStatsが空）想定
  const root = render();
  expect(rowOrder(root)).toEqual([squatLabel, benchPressLabel]);
});

test('操作なしでも、実績データがあればデフォルトの「最近使った順」（lastUsedAt降順）で並ぶ', () => {
  mockUseExerciseUsageStats.mockReturnValue(
    new Map([
      [benchPress.id, { recentUsageCount: 1, lastUsedAt: 100 }],
...
test('並び替えを「よく使う順」に変更すると、recentとは逆順（使用実績の多い種目が上）に並び替わる', () => {
  // benchPress: recentUsageCountは少ないがlastUsedAtは新しい / squat: 逆
  // → recent(デフォルト)ではbenchPressが上、frequentに切り替えるとsquatが上になり、
  ...
  const findSortTrigger = () =>
    root.findAllByType(TouchableOpacity).find((t) => (t.props.accessibilityLabel as string)?.startsWith('並び替え: '))!;
  act(() => {
    findSortTrigger().props.onPress();
  });
  act(() => {
    findMenuItem(root, 'よく使う順')!.props.onPress();
  });

  expect(findSortTrigger().props.accessibilityLabel).toBe('並び替え: よく使う順');
  expect(rowOrder(root)).toEqual([squatLabel, benchPressLabel]);
});
