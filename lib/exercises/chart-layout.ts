...
/**
 * 期間の外の記録から最初の点へ引く線の、描き始めの位置。プロットの左端に固定する。
 *
 * 期間外の点は縦軸のスケール（表示中の点だけで決めている）の外に出ることがあるので、左端での
 * yがはみ出す分は上下に丸める。**線分が枠を横切る位置で切ってはいけない。** 前回2kg→今回50kg
 * のように差が大きいと、線はすぐ下端にぶつかって右端の数pxの切れ端になり、「1点だけだと
 * グラフに見えない」という直したかった状態に戻ってしまう（実機で確認）。
 *
 * 丸めた分だけ左端のyは実際の値ではなくなるが、そもそも期間外の点は描いていないので線の左端は
 * 特定の日を指しておらず、正確な差は内訳カードの「前回比」に出ている。差が一定を超えると
 * 全て左下（左上）の隅からになるが、大きく動いたこと自体は伝わる。
 *
 * 期間外の1点を縦軸のスケールに含める案は採らない。2kg〜50kgのようなレンジになると
 * きりのいい目盛りに丸められず、目盛りが -5.2 / 26 / 57.2 のような半端な値に落ちる。
 */
function clipLeadIn(
  ...
  // 期間外の点が左端より内側（INSETの内）に来るほど直前の日なら、そこで止める
  const t = Math.min(1, (bounds.left - to.x) / dx);
  if (!(t > 0)) return null;

  const y = to.y + (from.y - to.y) * t;
  return {
    x: to.x + dx * t,
