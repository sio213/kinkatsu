...
// materializeReminderOccurrence（lib/notifications/scheduled-workout-scheduler.ts、種目カード
// タップ時にリマインダー予定をscheduledWorkoutsとして実体化する）から呼ばれる。関数名・
// テーブル名が「スキップ」のままなのは、内部的には「その日のこの発火は無かったことにする」
// マーカーとしての役割が変わっていないため。スキップ記録の保存と、該当日の通知キャンセルを
// セットで行う。通知キャンセルが失敗してもスキップ自体は成立させたい(表示が消えることの方が
// 優先)ため、通知側のエラーはcatchして握りつぶす。
// 既にスキップ済みの日への二重呼び出し(種目カード連打・useLiveQuery再購読前の再タップ等)は
...
// materializeReminderOccurrenceが、実体化（手動予定の追加）に失敗した際の内部ロールバック専用
// として呼ぶのみ（ユーザーが直接起動する導線は無い）。スキップ記録の削除と、未来日に限り
// 該当日の通知再登録をセットで行う
