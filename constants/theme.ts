...
/**
 * アイコンサイズのトークン。役割が同じアイコンは画面が違っても同じ大きさにする
 * （CLAUDE.md「タイポグラフィ・共通コンポーネント」の考え方をアイコンにも適用する）。
 * 汎用のサイズ表ではなく、揃っていないと目立つ役割にだけ名前を付けること。
 */
export const IconSizes = {
  /**
   * カード・行の右端に置く「タップすると次の画面へ進む」ことを示すchevron。デザイン確認値。
   * デザイン案の内訳カード（モックA）が chevron_right を 19px で描いているのに合わせる
   * （種目一覧カードだけは文字の「›」U+203A 20px で描かれているが、文字とアイコンフォントでは
   * 同じpxでも見た目の大きさが違うため、アイコン側の19pxに揃える）。
   * 種目一覧・記録の内訳カード・過去の記録カード・カレンダーの種目カード・ルーティンの
   * 種目行/ピッカー・過去のトレーニングカード・セッションカード・リマインダー欄・
   * トレーニング開始選択の行で共通。以前は18/19pxのIconSymbolと20pxの「›」文字が混在していた。
   * 折りたたみの開閉を示すchevron（回転するもの・14px）は役割が違うので含めない。
   */
  cardChevron: 19,
  /**
   * テキストリンクに添えるchevron（「すべての記録を見る >」）。デザイン確認値。
   * カード右端の20pxをそのまま置くと、12pxの文字に対して大きく見えてしまう。
   */
  inlineChevron: 16,
} as const;
