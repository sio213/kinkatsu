...
import { serializeFormPoints } from '@/lib/exercises/form-points';
...
export function useExercises() {
  ...
  const addExercise = useCallback(async (values: ExerciseFormValues) => {
  ...
  const updateExercise = useCallback(async (id: number, values: Partial<ExerciseFormValues>) => {
    const { formPoints, ...rest } = values;
    await db
      .update(exercises)
      .set({
        ...rest,
        ...(formPoints !== undefined ? { formPoints: serializeFormPoints(formPoints) } : {}),
        updatedAt: Date.now(),
      })
