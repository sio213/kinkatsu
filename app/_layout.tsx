    ...
    <GestureHandlerRootView style={styles.gestureRoot}>
      <ThemeProvider value={colorScheme === 'dark' ? DarkTheme : DefaultTheme}>
        ...
        >
          ...
          {/* ルーティンを更新の差分確認。タイトルはルーティン名をサブタイトルに出すため画面側で組む */}
          <Stack.Screen name="routine/update/[id]" options={{ title: '' }} />
          ...
          {/* 直接予定の種目一覧・目標セットをまとめて編集する画面（2026-07-20新設）。
              ...
              対象日時をヘッダーに動的表示するため、画面側でStack.Screen optionsを上書きする */}
          <Stack.Screen name="calendar/schedule-workout-edit" options={{ title: '' }} />
          ...
          <Stack.Screen name="calendar/schedule-workout-history-picker" options={{ title: '過去の記録' }} />
          {/* 画面3はヘッダーに日付を動的表示するため、画面側でStack.Screen optionsを上書きする */}
          <Stack.Screen name="calendar/schedule-workout-history-load" options={{ title: '' }} />
          <Stack.Screen
            name="modal"
