...
type Props = {
  ...
  metricRow: {
    ...
    unit: string;
    /**
     * ラベルに添える補足（「(左右合計)」）。セット行には片手の重量が出ているのに総重量だけが
     * 2倍になる種目で、その理由を数字と同じ場所に置くために使う。グラフ下の注記だけだと
     * スクロールで視野から外れ、カードの中に食い違う数字だけが残る（@designer指摘）
     */
    note?: string;
  } | null;
  ...
  return (
    ...
    <TouchableOpacity
      ...
      accessibilityLabel={
        ...
        (metricRow
          ? `、${metricRow.label}${metricRow.note ? ` ${metricRow.note}` : ''} ${metricRow.value}${metricRow.unit}`
          : '')
      }
        ...
        {metricRow && (
          <View style={styles.metricRow}>
            <Text style={styles.metricLabel}>
              {metricRow.label}
              {metricRow.note && <Text style={styles.metricNote}> {metricRow.note}</Text>}
            </Text>
...
const styles = StyleSheet.create({
  ...
  // ラベルへの補足はラベルより一段弱く。単位を値より弱く置くのと同じ作法
  metricNote: { ...Typography.captionCompact, color: Colors.textMuted },
