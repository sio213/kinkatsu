    ...
    <View>
      {failed ? (
        ...
        <ExerciseProgressChart
          ...
          // 横ドラッグは種目送りに使うため、グラフ側のスクラブは切る。点の選択はタップだけになる
          scrubbable={false}
        />
      )}
 ...
 *
 * **スワイプはグラフの上を含めブロック全体で効く。** グラフ本体は元々横ドラッグを点の選択に
 * 使っているが、サマリーではそれを切っている（scrubbable={false}）。グラフがブロックの過半を
 * 占めるため、ここが横ドラッグを握っていると種目送りの当たり判定が全体の1〜2割しか残らず、
 * 「スワイプが効かない」と読まれていた。この画面の主目的は種目を横断して眺めることで、
 * 履歴を指でなぞって探るのは種目詳細の役割、という切り分け（@designer・@user-advisor 合意）。
 * 点の選択はタップで残る（点を狙わなくても最寄りにスナップする）。
 *
